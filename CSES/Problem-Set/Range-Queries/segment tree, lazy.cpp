...
struct segment{
    ...
    void upd_node(int u, int u_val, int cnt, int flag){
        ...
        if(flag == 1){
            if(bit[u].lz == 2){
                ...
                bit[u].vlz += u_val;
                bit[u].cntlz += cnt;
                bit[u].val += (r - l) * u_val + (r - l) * (r - l - 1) * cnt / 2;
            }
        }
        else if(flag == 2){
            bit[u].lz = flag;
    ...
    void prop(int u){
        int v = u << 1;
        if((v ^ 1) < N && bit[v ^ 1].rng.s0){
            upd_node(v, bit[u].vlz, bit[u].cntlz, bit[u].lz);
            upd_node(v ^ 1, bit[u].vlz, bit[u].cntlz, bit[u].lz);
        }
        else if(v < N && bit[v].rng.s0)
            upd_node(v, bit[u].vlz, bit[u].cntlz, bit[u].lz);
        bit[u].cntlz = bit[u].vlz = bit[u].lz = 0;
        return;
    }
 
    void update(int lq, int rq, int flag, int u = 1){
        if(N <= u)
            return;
        int l = bit[u].rng.f0 , r = bit[u].rng.s0;
        if(l == r || r <= lq || rq <= l)
